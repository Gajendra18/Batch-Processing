...
public class BatchServiceImpl implements BatchService {
    ...
    public Long runJob(List<MultipartFile> multipartFile, String params) throws BatchException {
        ...
        String[] files = batchPartitionHelper.getAllFiles(inputFiles);
        int numberOfWorkers = 0;
        if (files.length == 1) {
            List<Map<String, Integer>> partitionPayloads = batchPartitionHelper.getStartingIndex(files[0], jobParamsObj.getPartitionSize());
    ...
    public Boolean stopJob(Long id) throws BatchException {
        try {
            Optional<JobExecution> optionalJobExecution = Optional.ofNullable(explorer.getJobExecution(jobOperator.getExecutions(id).get(0)));
            ...
            if( jobExecution.getStepExecutions().size() == 0){
                ...
                return jobOperator.stop(jobOperator.getExecutions(id).get(0));
            }
    ...
    public void startupClean() throws BatchException {
        ...
        for (JobInstance id : instanceIds) {
            ...
            if ((jobExecution.getStatus().equals(BatchStatus.STARTED) || jobExecution.getStatus().equals(BatchStatus.STOPPING))) {
                int readCount = 0;
                int writeCount = 0;
            ...
            updateInCompleteWorkers(jobExecution.getId());
        }
    }
...
//			if ((jobExecution.getStatus().equals(BatchStatus.STARTED) || jobExecution.getStatus().equals(BatchStatus.STARTING) || jobExecution.getStatus().equals(BatchStatus.STOPPING))
//					&& jobExecution.getCreateTime().before(jvmStartTime)) {
//				int readCount = 0;
//				int writeCount = 0;
//				StepExecution masterStep = null;
//				jobExecution.setEndTime(new Date());
...
//							se.setExitStatus(ExitStatus.FAILED);
//							jobRepository.update(se);
//						} else if (se.getStatus().equals(BatchStatus.COMPLETED)) {
//							count++;
//						}
//					}
//				}
...
//			updateInCompleteWorkers(jobExecution.getId());
    ...
    public Boolean scheduleJob(List<MultipartFile> multipartFile, String params) throws BatchException {
        ...
        String[] files = batchPartitionHelper.getAllFiles(inputFiles);
        long numberOfWorkers = 0;
        if (files.length == 1) {
            List<Map<String, Integer>> partitionPayloads = batchPartitionHelper.getStartingIndex(files[0], scheduleDetailsObj.getPartitionSize());
            numberOfWorkers = partitionPayloads.size();
        } else {
    ...
    @Transactional
    public void updateInCompleteWorkers(long jobExecutionId) {
        List<WorkerInfo> incompleteWorkers = workerRepository.findIncompleteWorkers(jobExecutionId);
        ...
        if (writeCount != 0){
            ...
            for(Document doc: results){
                for (String key : doc.keySet()){
                    log.info(key + " " + doc.get(key).getClass().getSimpleName());
